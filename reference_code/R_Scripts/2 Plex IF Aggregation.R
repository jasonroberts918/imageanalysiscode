...
path.det = "C:\\Analysis_Studies\\22-771 IMVAX FoxP3_CD4 QP\\detection results\\"
path.ano =  "C:\\Analysis_Studies\\22-771 IMVAX FoxP3_CD4 QP\\annotation results\\"
...
QPRed = "FoxP3" #QP Red
QPGreen = "CD4" #QP Green

#QP string names
cell_name = "Object"
TRITC_posName = "Green Negative: Red Positive"
Cy5_posName = "Green Positive: Red Negative"
...
for(f in 1:length(filelist)){
  ...
  QPred_only <- subset(Read.Data, Read.Data$Class == "Green Negative: Red Positive")
  QPgreen_only <- subset(Read.Data, Read.Data$Class == "Green Positive: Red Negative")
  QPboth <- subset(Read.Data, Read.Data$Class == "Green Positive: Red Positive")
  QPneg <- subset(Read.Data, Read.Data$Class == "Green Negative: Red Negative")
  ...
  totalCellArea <- NegativeArea+posRedArea+posGreenArea+posBothArea
  ...
  redIntensity <- mean(QPred_only$Cell..Red.mean)
  greenIntensity<-mean(QPgreen_only$Cell..Green.mean)
  
  totalCells <- nrow(QPred_only) + nrow(QPgreen_only) + nrow(QPboth) + nrow(QPneg)
  ...
  DataDraft[k,2] <- nrow(QPgreen_only) / totalCellArea
...
write.csv(DataDraft,paste("22-771 CD4 FoxP3 Data",f,".csv"), row.names = F)
write.xlsx(DataDraft, paste("22-771 CD4 FoxP3 Data Excel",f,".xlsx"))
