...
path.det = "C://Analysis_Studies//23-690 UNCCH QP Inflammation//detection results"
path.ano = "C://Analysis_Studies//23-690 UNCCH QP Inflammation//annotation results"
setwd(path.det)
...
for(f in 1:length(filelist)){
  ...
  NegativeArea <- sum(Negative$Cell..Area) / 1000000 #division for conversion from um^2 to mm^2
  PositiveArea <- sum(Positive$Cell..Area) / 1000000 #division for conversion from um^2 to mm^2
  ...
  DataDraft[k,4] <- PositiveArea
  DataDraft[k,5] <- nrow(Positive) / (nrow(Positive) + nrow(Negative)) *100
  DataDraft[k,6] <- nrow(Positive) + nrow(Negative)
...
write.csv(DataDraft,paste("23-690 Inflammation Data",f,".csv"))