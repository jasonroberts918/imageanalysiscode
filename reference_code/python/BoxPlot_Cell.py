...
def load_data(file_path):
    ...
    if 'Sample' not in data.columns or 'Positive_Cell_Percentage' not in data.columns:
        raise ValueError("Required columns 'Sample' and 'Positive_Cell_Percentage' not found.")
...
def create_boxplot(data, group_by='Sample', custom_palette=None):
    ...
    ax.set_title(f'FLAG Positive Fiber Percentage by {group_by}')
    ax.set_xlabel(group_by)
    ax.set_ylabel('Positive Fiber Percentage')
